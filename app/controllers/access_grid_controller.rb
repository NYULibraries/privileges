...
class AccessGridController < ApplicationController
  ...
  def index_patron_statuses
    ...
    respond_to do |format|
      format.html
      format.json { render :json => patron_statuses_results, :layout => false }
    end
  end
  ...
  def show_patron_status
    ...
    respond_to do |format|
      ...
      format.json { render :json => {:patron_status_permissions => @patron_status_permissions, :sublibrary => @sublibrary, :patron_status => @patron_status}, :layout => false }
	    format.js { render :layout => false } # Render permissions without reload
    end
  end
  ...
  def search
    ...
    respond_to do |format|
	    ...
	    format.html { redirect_to patron_status_link(@patron_statuses.results[0].id, @patron_statuses.results[0].web_text) if @patron_statuses.results.count == 1 }
  ...
  def redirect_to_patron_status
    ...
    if !session[:redirected_user] && !current_user.nil?
      ...
      unless @patron_status.nil?
        redirect_to patron_status_link(@patron_status.primary_key, @patron_status.stored(:web_text))
      end
    end
  end
